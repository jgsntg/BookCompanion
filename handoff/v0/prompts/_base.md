...
Your extraction should serve those queries. Be selective — a strong chapter might yield 3 items per category, not 15. Quality over coverage. Empty arrays are acceptable and often correct.
...
Return a single JSON object. Do not include any prose outside the JSON, and do not wrap it in markdown fences.
...
1. **Be selective.** Empty arrays are correct when nothing rises to the bar. Don't pad. Don't invent.
...
5. **Do not summarize plot in `summary`.** The reader is listening to the audiobook — they know what happens. Plot beats belong in `key_events` (fiction only). The `summary` should describe what the chapter is *trying to do*.
